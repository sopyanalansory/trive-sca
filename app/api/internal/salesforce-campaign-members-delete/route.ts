...
import {
  deleteCampaignMembersBySfIds,
  extractCampaignMemberIdsFromDeletePayload,
} from "@/lib/salesforce-campaign-members-webhook-delete";
import { apiLogger, logRouteError } from "@/lib/logger";

const log = apiLogger("internal:salesforce-campaign-members-delete");
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ids = extractCampaignMemberIdsFromDeletePayload(body);
    if (ids.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: "VALIDATION",
          message:
            "Payload harus berisi Id, ids: string[], campaignMembers: [{ Id }], atau array objek dengan field Id.",
        },
        { status: 200 }
      );
    }

    const result = await deleteCampaignMembersBySfIds(ids);
    return NextResponse.json({
      success: true,
    ...
    logRouteError(
      ...
      request,
      error,
      "Salesforce campaign members webhook delete failed"
    );
    return NextResponse.json(
      {
        success: false,
        error: "INTERNAL_ERROR",
        message: "Gagal menghapus data campaign members.",
      },
      { status: 200 }
    );
  }
