...
// Run meta_text migration for market_updates table
export async function runMetaTextMigration() {
  try {
    console.log('Running meta_text migration for market_updates table...');
    
    const sqlFile = readFileSync(
      join(process.cwd(), 'lib', 'add-meta-text-to-market-updates.sql'),
      'utf8'
    );
    ...
    console.log('Meta text migration completed successfully');
    console.log('Column "meta_text" has been added to market_updates table');
  } catch (error: any) {
    console.error('Error running meta_text migration:', error);
...
export default runMetaTextMigration;
